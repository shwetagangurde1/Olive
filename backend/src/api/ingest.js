...
const InferenceLogSchema = z.object({
  ...
  stream_chunks: z.number().int().min(0).optional(),

  estimated_cost_microcents: z.number().int().min(0).optional(),
...
router.post('/log', async (req, res) => {
  ...
  const inputResult = data.input_text
    ? redact(data.input_text)
    : { redacted: null, hasPii: false };

  const outputResult = data.output_text
    ? redact(data.output_text)
    : { redacted: null, hasPii: false };

  const inputPreview = safePreview(
    inputResult.redacted || data.input_text
  );

  const outputPreview = safePreview(
    outputResult.redacted || data.output_text
  );
  ...
  try {
    const result = await query(
      ...
      [
        ...
        data.request_ended_at,

        data.latency_ms,
        data.time_to_first_token_ms,
...
router.post('/batch', async (req, res) => {
  ...
  try {
    const results = await Promise.allSettled(
      logs.map((log) =>
        fetch(
          `http://localhost:${process.env.PORT || 3001}/api/ingest/log`,
          {
            method: 'POST',
            headers: {
